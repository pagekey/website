...
Start with the plaintext files, the **data**, at the absolute center, rather than a server/web app codebase. Work out from there - first, CLI to easily manage those files locally. Then, a user interface that can run locally. Then, a server to provide the same functionality to remote clients that can't really manage their own files (for example, it's REALLY tough to install Git or Python on an iPhone).

Finally, the web and mobile clients to interact with that server. The result? You have one codebase that can basically run on everything and replicate the same functionality everywhere.
...
After creating some sample data, the CLI should be the next step. It's by far the easiest way to interface with the user. You don't have to worry about how things are presented - if they're used to interacting with the terminal, then they'll be able to figure it out.

### Server

At that point, you should be able to abstract the code to handle CLI inputs away from the real file operations. Then, you can implement a server that exposes an API to perform the same operations that the CLI does. The only difference is that it's triggered by a web request instead of a user interacting with the terminal.
